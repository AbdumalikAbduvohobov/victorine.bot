...
async def level_1(message: Message):
    question = f"{random.randint(11, 21)} {random.choice(['+', '-', '*'])} {random.randint(1, 11)}"
    await message.answer(text=f"SAVOL: {question} = ?")

@router.message(F.text == "Level 3️⃣")
async def level_1(message: Message):
    question = f"{random.randint(21, 31)} {random.choice(['+', '-', '*'])} {random.randint(1, 11)}"
    await message.answer(text=f"SAVOL: {question} = ?")

@router.message(F.text == "Level 4️⃣")
async def level_1(message: Message):
    question = f"{random.randint(31, 51)} {random.choice(['+', '-', '*'])} {random.randint(1, 11)}"
    await message.answer(text=f"SAVOL: {question} = ?")